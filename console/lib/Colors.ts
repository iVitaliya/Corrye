...
export class Colors {
  ...
  public constructor(options: ColorsFormatOptions = {}) {
    ...
    const { opening, closing } = Constructor.text(
      options.text,
      Constructor.background(
        options.background,
        Constructor.style(options.style!),
      ),
    );

    this.opening = Constructor.useColors && opening
      ? `\u001B[${opening.join(";")}m`
      : "";
    this.closing = Constructor.useColors && closing
      ? `\u001B[${closing.join(";")}m`
      : "";
  }
  ...
  private static style(
    styles: Style | Style[],
    { opening = [], closing = [] }: ColorsFormatData = {},
  ): ColorsFormatData {
    if (styles) {
